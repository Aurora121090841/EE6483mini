# import torch
# import argparse
# from config import DATA_CONFIG, MODEL_CONFIG, TRAIN_CONFIG, IMBALANCE_CONFIG  # 补充导入所有配置
# from data_utils import get_dataloaders
# from trainer import train_model, test_model
# from models import get_model  # 直接使用已有的models.py，不再重复实现

# # --------------------------
# # 解析命令行参数（支持动态调整配置）
# # --------------------------
# def parse_args():
#     parser = argparse.ArgumentParser(description='纯在线图像分类（支持类别不平衡开关）')
#     parser.add_argument('--dataset', type=str, choices=['cifar10', 'catdog'], default='catdog',
#                         help='数据集类型（默认：catdog）')
#     parser.add_argument('--model_type', type=str, default=MODEL_CONFIG['model_type'],
#                         help='模型类型（默认：resnet18）')
#     parser.add_argument('--imbalance_enable', action='store_true', default=IMBALANCE_CONFIG['enable'],
#                         help='是否启用类别不平衡处理（默认：False）')
#     parser.add_argument('--imbalance_strategy', type=str, choices=['oversample', 'weighted_loss'],
#                         default=IMBALANCE_CONFIG['strategy'], help='不平衡处理策略')
...
#     IMBALANCE_CONFIG['enable'] = args.imbalance_enable
...
#     MODEL_CONFIG['num_classes'] = len(class_names)  # 更新类别数

#     # 2. 获取模型（使用models.py中的get_model）
#     model = get_model(
...
#     ).to(device)
#     print(f'使用模型：{MODEL_CONFIG["model_type"]}，类别数：{MODEL_CONFIG["num_classes"]}')
